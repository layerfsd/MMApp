...
export default {
    ...
    beforeCreate() {
        // 加入缓存
        this.$store.dispatch('pushKeepAlive', 'reset-security');
    },
    created() {
    ...
    methods: {
        ...
        getQuestions() {
            this.$nextTick(() => {
                // 获取3个问题
                this.showIndicator();
                this.serverApi.queryMarchantSecurity(null, {returnToPage: '/resetSecurity'}, (error, data, wentlogin) => {
                    this.hideIndicator();
                    if(wentlogin){
                        return;
                    }
                    ...
                    console.log('问题号： ' + this.satefyProblemNum)
                    for (let i = 0; i < ret.length; i++) {
                        for (let j = 0; j < questions.length; j++) {
                            if (ret[i] == questions[j].id) {
                                this.randomQuestions.push(questions[j]);
                            }
                        }
                    }
                    console.log('随机问题： ' + this.randomQuestions)
                    ...
                    for (let i = 0; i < questions.length; i++) {
                        ...
                        flag && this.otherQuestions.push(questions[i]);
                    }

                    console.log('剩余问题：' + this.otherQuestions)
                });
            });
        },
        // 检查输入内容
        ...
        next() {
            ...
            this.serverApi.modifySecurityRecord({
                ...
                this.showToast(this.$t('info_重置密保问题成功'));
                setTimeout(() => {
