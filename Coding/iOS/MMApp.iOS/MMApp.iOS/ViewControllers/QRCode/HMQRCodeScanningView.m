...
CGFloat const HMQRCodeScanningLineAnimation = 0.05;

@interface HMQRCodeScanningView ()
...
@implementation HMQRCodeScanningView
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    UIImage *left_image = [UIImage imageNamed:@"HMQRCode.bundle/QRCodeLeftTop"];
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"HMQRCode.bundle/QRCodeRightTop"];
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"HMQRCode.bundle/QRCodeLeftBottom"];
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"HMQRCode.bundle/QRCodeRightBottom"];
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.image = [UIImage imageNamed:@"HMQRCode.bundle/QRCodeScanningLine"];
...
- (void)addTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:HMQRCodeScanningLineAnimation target:self selector:@selector(timeAction) userInfo:nil repeats:YES];
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        flag = NO;
        [UIView animateWithDuration:HMQRCodeScanningLineAnimation animations:^{
            frame.origin.y += 5;
