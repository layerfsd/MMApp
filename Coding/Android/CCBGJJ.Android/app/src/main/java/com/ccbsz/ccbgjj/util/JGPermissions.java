...
public class JGPermissions {
    private static JGPermissions mJGPermissions = null;
    private JGPermissionsListener mListener = null;
    public static JGPermissions getInstance(){
        if(mJGPermissions == null){
            mJGPermissions = new JGPermissions();
        }
    ...
    public void checkAndRequestPermissions(Context context, String[] psemissions, JGPermissionsListener listener){
        ...
        if (checkList.size() > 0) {
            ...
            ActivityCompat.requestPermissions((Activity) context, checks, JGGlobals.PERMISSIONS_REQUEST_CDOE);
        }else{
            if(mListener != null){
