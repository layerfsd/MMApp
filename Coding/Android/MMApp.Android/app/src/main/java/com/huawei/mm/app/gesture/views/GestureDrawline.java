package com.huawei.mm.app.gesture.views;
...
import com.huawei.mm.app.gesture.util.DisPlayUtils;
import com.huawei.mm.app.gesture.util.GestureGlobals;
import com.huawei.mm.app.gesture.util.GesturePoint;
import com.huawei.mm.app.gesture.util.MD5Utils;
...
public class GestureDrawline extends View {
    ...
    public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
        ...
        screenDispaly = DisPlayUtils.getScreenDisplay(context);
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (currentPoint != null) {
                    currentPoint.setPointState(GestureGlobals.POINT_STATE_SELECTED);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointAt == null || currentPoint.equals(pointAt) || GestureGlobals.POINT_STATE_SELECTED == pointAt.getPointState()) {
                    ...
                    pointAt.setPointState(GestureGlobals.POINT_STATE_SELECTED);

                    // 判断是否中间点需要选中
                    GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
                    if (betweenPoint != null && GestureGlobals.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
                        // 存在中间点并且没有被选中
                        ...
                        // 设置中间点选中
                        betweenPoint.setPointState(GestureGlobals.POINT_STATE_SELECTED);
                        // 赋值当前的point;
                        currentPoint = pointAt;
                    } else {
            ...
            case MotionEvent.ACTION_UP:// 当手指抬起的时候
                if (isVerify) {
                    ...
                    if (passWord.equals(MD5Utils.encryptStringToMd5(passWordSb.toString())) || passWord == null) {//如果传入的passwrod为null任意绘制都算成功，让js去服务器比对
                        // 代表用户绘制的密码手势与传入的密码相同
    ...
    final class clearStateRunnable implements Runnable {
        public void run() {
            ...
            for (GesturePoint p : list) {
                p.setPointState(GestureGlobals.POINT_STATE_NORMAL);
            }
            invalidate();
            isDrawEnable = true;
        }
    ...
    private void drawErrorPathTip() {
        ...
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
            pair.first.setPointState(GestureGlobals.POINT_STATE_WRONG);
            pair.second.setPointState(GestureGlobals.POINT_STATE_WRONG);
            canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
                    pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
        }
        invalidate();
    ...
    public interface GestureCallBack {
         ...
         * @param input
         */
        public abstract void checkedSuccess(String input);
         ...
         * @param input
         */
        public abstract void checkedFail(String input);
    }
